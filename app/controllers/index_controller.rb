class IndexController < ApplicationController
	...
	def detail
		...
		@away_starters.each do |away_starter|
			...
			selected_player = @away_players_search.where("player_name = ?", away_starter.player_name).first unless selected_player
			if selected_player
		...
		@home_starters.each do |home_starter|
			...
			selected_player = @home_players_search.where("player_name = ?", home_starter.player_name).first unless selected_player
		...
		@referee_filter.each_with_index do |referee_filter_element, index|
			if referee_filter_element[0] != '-'
				...
				@referee_filter_results.push([
					referee_filter_result.map {|stat| stat.tp_1h.to_f }.sum.round(2),
					referee_filter_result.map {|stat| stat.tp_2h.to_f }.sum.round(2),
					referee_filter_result.map {|stat| stat.away_pf.to_f }.sum.round(2) + referee_filter_result.map {|stat| stat.home_pf.to_f }.sum.round(2),
					referee_filter_result.map {|stat| stat.away_fta.to_f }.sum.round(2) + referee_filter_result.map {|stat| stat.home_fta.to_f }.sum.round(2),
