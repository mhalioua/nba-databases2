class FilterController < ApplicationController
  ...
  def filter
    ...
    @games = Game.where("game_date between ? and ? AND id >= 26796", Date.strptime(@game_start_index).beginning_of_day, Date.strptime(@game_end_index).end_of_day)
                  .or(Game.where("game_date between ? and ? AND id <= 26573 AND id >= 25261", Date.strptime(@game_start_index).beginning_of_day, Date.strptime(@game_end_index).end_of_day)).order('id DESC')
  end
end