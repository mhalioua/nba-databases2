namespace :cbb do
	...
	task :getDate, [:game_date] => [:environment] do |t, args|
      ...
      puts url
        ...
        player = CbbPlayer.find_or_create_by_player_name(player_name: player_name)
        player.update(link: link, cbb_team_id: away_team.id)
				...
				record = CbbRecord.find_or_create_by_cbb_player_id_and_cbb_game_id(cbb_player_id: player.id, cbb_game_id: game.id)
