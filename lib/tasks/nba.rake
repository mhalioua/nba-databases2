namespace :nba do
	...
	task :getUpdatePoss => [:environment] do
        ...
        sum_pf = 0
				...
				mins_min = 100
				mins_max = 0
				last_players = Player.where("game_date >= ? AND game_date <= ? AND link = ?", Date.new(2017, 10 ,20), player.game_date, player.player_name).or(Player.where("game_date <= ? AND player_name = ?", Date.new(2017, 6 ,18), player.link)).order('game_date DESC')
