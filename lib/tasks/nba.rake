namespace :nba do
  ...
  task :getFiltervalue => :environment do
	...
	task :getUpdateTG => [:environment] do
		include Api
		games = Nba.where("game_date between ? and ?", (Date.today - 3.days).beginning_of_day, Time.now-5.hours)
		puts games.size
		games.each do |game|
  ...
  task :filterNbaPast => :environment do
    ...
    games = Nba.where("game_date between ? and ?", Date.new(2018, 10, 15), Date.new(2018, 11, 1))
    puts games.size
    games.each do |game|
      filters.each_with_index do |filter, index|
        search_string = []
