    ...
    0.upto(1) do |type|
      ...
      times.each_with_index do |time_element, index|
        ...
        away_players.children.each_with_index do |away_player, index|
          ...
          unless starter = Starter.find_by(time: time, team: away_team, index: index/2+1)
            starter = Starter.create(time: time, team: away_team, index: index/2+1)
          end
        ...
        home_players.children.each_with_index do |home_player, index|
          ...
          unless starter = Starter.find_by(time: time, team: home_team, index: index/2+1)
            starter = Starter.create(time: time, team: home_team, index: index/2+1)
          end
          starter.update(position: position, player_name: player_name)
        end
      end
