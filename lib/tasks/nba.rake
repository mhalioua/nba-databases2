  ...
  task :tvstation => [:environment] do
	...
	task :getUpdatePoss => [:environment] do
        ...
        sum_pf = 0
				...
				mins_min = 100
				mins_max = 0
				last_players = Player.where("game_date >= ? AND game_date <= ? AND link = ?", Date.new(2017, 10 ,20), player.game_date, player.link).or(Player.where("game_date <= ? AND link = ?", Date.new(2017, 6 ,18), player.link)).order('game_date DESC')
    ...
    0.upto(1) do |type|
      ...
      times.each_with_index do |time_element, index|
        ...
        away_players.children.each_with_index do |away_player, index|
          ...
          starter = Starter.find_or_create_by(time: time.to_s, team: away_team)
          starter.update(position: position, player_name: player_name, index: (index - 1)/2)
        end
        home_players.children.each_with_index do |home_player, index|
          ...
          starter = Starter.find_or_create_by(time: time.to_s, team: home_team)
          starter.update(position: position, player_name: player_name, index: (index - 1)/2)
        end
      end
