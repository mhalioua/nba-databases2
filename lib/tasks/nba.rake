namespace :nba do
  ...
  task :getLineFix => :environment do
    Rake::Task["nba:getFirstLinesFix"].invoke
    Rake::Task["nba:getFirstLinesFix"].reenable

    link = "https://classic.sportsbookreview.com/betting-odds/nba-basketball/2nd-half/?date="
    Rake::Task["nba:getSecondLinesFix"].invoke("second", link)
    Rake::Task["nba:getSecondLinesFix"].reenable

    link = "https://classic.sportsbookreview.com/betting-odds/nba-basketball/?date="
    Rake::Task["nba:getSecondLinesFix"].invoke("full", link)
    Rake::Task["nba:getSecondLinesFix"].reenable

    link = "https://classic.sportsbookreview.com/betting-odds/nba-basketball/totals/1st-half/?date="
    Rake::Task["nba:getSecondLinesFix"].invoke("firstTotal", link)
    Rake::Task["nba:getSecondLinesFix"].reenable

    link = "https://classic.sportsbookreview.com/betting-odds/nba-basketball/totals/2nd-half/?date="
    Rake::Task["nba:getSecondLinesFix"].invoke("secondTotal", link)
    Rake::Task["nba:getSecondLinesFix"].reenable

    link = "https://classic.sportsbookreview.com/betting-odds/nba-basketball/totals/?date="
    Rake::Task["nba:getSecondLinesFix"].invoke("fullTotal", link)
    Rake::Task["nba:getSecondLinesFix"].reenable
  end

  task :getFirstLinesFix => [:environment] do
    include Api
    games = Nba.all
    puts "----------Get First Lines----------"

    index_date = Date.new(2018, 10, 15)
    while index_date <= Date.new(2018, 10, 22)  do
      game_day = index_date.strftime("%Y%m%d")
      puts game_day
      url = "https://classic.sportsbookreview.com/betting-odds/nba-basketball/1st-half/?date=#{game_day}"
      doc = download_document(url)
  ...
  task :getSecondLinesFix, [:type, :game_link] => [:environment] do |t, args|
    include Api
    games = Nba.all
    game_link = args[:game_link]
    type = args[:type]
  ...
  task :nbaplaybyplay => :environment do
    include Api
    games = Nba.where('away_foul_first is null')
    games.each do |game|
