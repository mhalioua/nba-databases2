namespace :nba do
  ...
  task :aws_upload => :environment do
    ...
    games.each do |game|
      ...
      if game.filters.present?
          ...
          obj = S3.object("images/#{game.game_id}.png")
  ...
  task :filterNbaTest => :environment do
    filters = [
      [false, false, false, false, false, false, false, false],
      [true, true, true, true, true, true, true, true],
      [false, true, true, true, true, true, true, true],
    ...
    games = Nba.where("game_date between ? and ?", Date.new(2017, 10, 16).beginning_of_day, Date.new(2018, 4, 12).end_of_day)
    puts games.size
    games.each do |game|
      filters.each_with_index do |filter, index|
        search_string = []
